...
/**
 * Transforms a {@link mx.com.cinepolis.digital.booking.model.BookingDO} to aBookingTo to use in reports
 * 
 * @author jcarbajal
 */
public class BookingDOToBookingTOReportTransformer implements Transformer
{
   ...
   * Constructor default
   */
  public BookingDOToBookingTOReportTransformer()
  {
   ...
   * @param language
   */
  public BookingDOToBookingTOReportTransformer( Language language )
  {
    ...
    if( object instanceof BookingDO )
    {
      ...
      if( CollectionUtils.isNotEmpty( bookingDO.getBookingSpecialEventDOList() ) )
      {
        ...
        for( BookingSpecialEventDO specialEvent : bookingDO.getBookingSpecialEventDOList() )
        {
          ...
          selectedScreenDOs.addAll( getScreensSpecialEvents( specialEvent.getBookingSpecialEventScreenDOList(),
            showsPerScreen, observations, statusPerScreen ) );
        }
        bookingTO.setStatus( (CatalogTO) new BookingStatusDOToCatalogTOTransformer( this.language )
  ...
  private List<ScreenDO> getScreens( List<BookingWeekScreenDO> bookingWeekScreens,
      ...
      Map<ScreenDO, CatalogTO> statusPerScreen )
  {
    List<ScreenDO> selectedScreenDOs = new ArrayList<ScreenDO>();
  ...
  private List<ScreenDO> getScreensSpecialEvents( List<BookingSpecialEventScreenDO> bookingWeekScreens,
      Map<ScreenDO, List<CatalogTO>> showsPerScreen, Map<ScreenDO, BookingObservationTO> observations,
      Map<ScreenDO, CatalogTO> statusPerScreen )
  {
    List<ScreenDO> selectedScreenDOs = new ArrayList<ScreenDO>();
    if( CollectionUtils.isNotEmpty( bookingWeekScreens ) )
    {
      for( BookingSpecialEventScreenDO bookingSpecialEventScreenDO : bookingWeekScreens )
      {
        List<CatalogTO> shows = new ArrayList<CatalogTO>();
        if( CollectionUtils.isNotEmpty( bookingSpecialEventScreenDO.getBookingSpecialEventScreenShowDOList() ) )
        {
          for( BookingSpecialEventScreenShowDO bookingWeekScreenShowDO : bookingSpecialEventScreenDO
        ...
        showsPerScreen.put( bookingSpecialEventScreenDO.getIdScreen(), shows );

        if( bookingSpecialEventScreenDO.getIdObservation() != null )
        {
          BookingObservationTO observation = getObservation(
            bookingSpecialEventScreenDO.getIdBookingSpecialEventScreen(),
            bookingSpecialEventScreenDO.getIdObservation() );
          observations.put( bookingSpecialEventScreenDO.getIdScreen(), observation );
        }
        CatalogTO status = (CatalogTO) new BookingStatusDOToCatalogTOTransformer( this.language )
            .transform( bookingSpecialEventScreenDO.getIdBookingStatus() );
        statusPerScreen.put( bookingSpecialEventScreenDO.getIdScreen(), status );
        selectedScreenDOs.add( bookingSpecialEventScreenDO.getIdScreen() );
      }
    }
    return selectedScreenDOs;
  }
